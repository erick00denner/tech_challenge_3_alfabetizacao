...
def _validate_domains(chunk: pd.DataFrame, year: int) -> None:
    ...
    if unexpected_loc_dif:
        raise RuntimeError(
            f"{year}: {LOCALIZACAO_DIFERENCIADA_COLUMN} contém valores inesperados: "
            f"{sorted(unexpected_loc_dif)}"
        )
...
def _aggregate_chunk(chunk: pd.DataFrame) -> pd.DataFrame:
    ...
    # Harmonização temporal de localização diferenciada:
    # 0 = não diferenciada; 1/2/3/8 = alguma localização diferenciada.
    loc_col = LOCALIZACAO_DIFERENCIADA_COLUMN
    parts.append(grouped[loc_col].count().rename(f"_{loc_col}_valid").to_frame())
    temp = chunk[KEY_COLUMNS].copy()
    temp["_indicator"] = (
        chunk[loc_col].isin(["1", "2", "3", "8"]).fillna(False).astype("int8")
    )
    counts = (
...
def _finalize_aggregation(accum: pd.DataFrame, year: int) -> pd.DataFrame:
    ...
    for col, categories in CATEGORICAL_COLUMNS.items():
        valid = accum[f"_{col}_valid"].replace(0, np.nan)
        name = col.lower()
        out[f"cobertura_{name}_pct"] = valid / qtd * 100
        for category in categories:
            out[f"pct_{name}_{category}"] = accum[f"_{col}_{category}_sum"] / valid * 100

    loc_col = LOCALIZACAO_DIFERENCIADA_COLUMN
    valid = accum[f"_{loc_col}_valid"].replace(0, np.nan)
    out["pct_localizacao_diferenciada"] = (
        accum[f"_{loc_col}_diferenciada_sum"] / valid * 100
    )
    out["cobertura_localizacao_diferenciada_pct"] = valid / qtd * 100

    for col in BINARY_COLUMNS:
        valid = accum[f"_{col}_valid"].replace(0, np.nan)
        name = col.lower()
        out[f"pct_{name}"] = accum[f"_{col}_sum"] / valid * 100
        out[f"cobertura_{name}_pct"] = valid / qtd * 100

    for col in QUANTITATIVE_COLUMNS:
        valid = accum[f"_{col}_valid"].replace(0, np.nan)
        name = col.lower()
        out[f"media_{name}"] = accum[f"_{col}_sum"] / valid
        out[f"total_{name}"] = accum[f"_{col}_sum"]
        out[f"cobertura_{name}_pct"] = valid / qtd * 100
    ...
    dispositivos = out["total_qt_desktop_aluno"] + out["total_qt_comp_portatil_aluno"] + out["total_qt_tablet_aluno"]
...
def process_year(year: int, input_dir: Path, output_dir: Path, chunk_size: int) -> dict:
    ...
    with ZipFile(zip_path) as zf:
        with zf.open(csv_name) as raw:
            reader = pd.read_csv(
                raw, sep=";", encoding="latin1", usecols=SELECTED_COLUMNS,
                dtype="string", keep_default_na=False, na_filter=False,
            ...
            for i, chunk in enumerate(reader, start=1):
                ...
                if i == 1 or i % 5 == 0:
                    print(f"[{year}] {rows_read:,} escolas processadas...")

    if accumulator is None:
        raise RuntimeError(f"{year}: nenhum registro foi processado.")
...
def write_manifest(output_dir: Path, results: list[dict], years: list[int]) -> Path:
    manifest = {
        ...
        "missing_data_policy": {
            ...
            "binary_proportions": "calculated only over schools with valid information",
            "quantitative_means": "calculated only over schools with valid information",
            "coverage_metrics": "created to retain information about missingness",
